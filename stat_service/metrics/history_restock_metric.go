...
func NewHistoryRestockMetric(db *gorm.DB, filter *selling_iface.StatFilter, trange *common.StatTimeRange) (*selling_iface.Metric, error) {
	var err error
...
func NewHistoryStockResolutionMetric(db *gorm.DB, filter *selling_iface.StatFilter, trange *common.StatTimeRange) (*selling_iface.Metric, error) {
	var err error
	result := selling_iface.HistoryStockResolutionMetric{
		TimeType: trange.Type,
		Items:    []*selling_iface.HistoryStockResolutionItem{},
	}

	var selects []string = []string{
		"count(iti.inv_transaction_id) as found_transaction_count",
		"sum(iti.count) as found_piece_count",
		"sum(iti.total) as found_piece_amount",
	}

	switch trange.Type {
	case common.StatTimeType_STAT_TIME_TYPE_DAY:
		selects = append(selects, "date_trunc('day', it.arrived) as t")
	case common.StatTimeType_STAT_TIME_TYPE_WEEK:
		selects = append(selects, "date_trunc('week', it.arrived) as t")
	case common.StatTimeType_STAT_TIME_TYPE_MONTH:
		selects = append(selects, "date_trunc('month', it.arrived) as t")
	case common.StatTimeType_STAT_TIME_TYPE_YEAR:
		selects = append(selects, "date_trunc('year', it.arrived) as t")
	}

	query := db.
		Table("inv_transactions it").
		Joins("left join inv_tx_items iti on iti.inv_transaction_id = it.id").
		Where("it.type = ?", db_models.InvTxAdjIn).
