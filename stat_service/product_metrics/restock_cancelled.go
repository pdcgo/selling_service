...
func createRestockCancelledQuery(db *gorm.DB, pfilter *selling_iface.ProductStatMetricFilter) *gorm.DB {
	...
	query := db.
		...
		Joins("join inv_timestamps its on its.tx_id = p.inv_transaction_id and its.status = 'cancel'").
...
func NewRestockCancelledMetric(db *gorm.DB) metric_base.ProductMetricBase {
	return &restockCancelledMetric{db: db}
}

func (m *restockCancelledMetric) ProcessSort(ctx context.Context, pfilter *selling_iface.ProductStatMetricFilter, psort *selling_iface.ProductMetricSort) ([]uint64, error) {
	var err error
	var productIds []uint64
	var sortField string

	query := createRestockCancelledQuery(m.db, pfilter)

	// sorting
	switch psort.GetRestockCancelledMetricSort() {
	case product_metric.RestockCancelledMetricSort_RESTOCK_CANCELLED_METRIC_SORT_LAST_CANCELLED:
		sortField = "max(its.timestamp) as sfield"
	case product_metric.RestockCancelledMetricSort_RESTOCK_CANCELLED_METRIC_SORT_TOTAL_AMOUNT:
		sortField = "sum(p.piece_amount + (p.piece_count * coalesce(rc.per_piece_fee, 0))) as sfield"
	case product_metric.RestockCancelledMetricSort_RESTOCK_CANCELLED_METRIC_SORT_PIECE_COUNT:
		sortField = "sum(p.piece_count) as sfield"
	case product_metric.RestockCancelledMetricSort_RESTOCK_CANCELLED_METRIC_SORT_TRANSACTION_COUNT:
		sortField = "count(p.inv_transaction_id) as sfield"
	case product_metric.RestockCancelledMetricSort_RESTOCK_CANCELLED_METRIC_SORT_TRANSACTION_AMOUNT:
...
func (m *restockCancelledMetric) FetchMetric(ctx context.Context, productIds []uint64, pfilter *selling_iface.ProductStatMetricFilter) (*selling_iface.ProductMetric, error) {
	var err error

	result := product_metric.RestockCancelledMetric{
		Data: map[uint64]*product_metric.RestockCancelledItem{},
	}

	resultList := []*product_metric.RestockCancelledItem{}
	selects := []string{
		"p.product_id",
	...
	return &selling_iface.ProductMetric{
		Data: &selling_iface.ProductMetric_RestockCancelledMetric{
			RestockCancelledMetric: &result,
		},
	}, err
}
