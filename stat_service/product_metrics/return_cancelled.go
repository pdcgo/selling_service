...
func createReturnCancelledQuery(db *gorm.DB, pfilter *selling_iface.ProductStatMetricFilter) *gorm.DB {
...
func NewReturnCancelledMetric(db *gorm.DB) metric_base.ProductMetricBase {
	return &returnCancelledMetric{db: db}
}

func (m *returnCancelledMetric) ProcessSort(ctx context.Context, pfilter *selling_iface.ProductStatMetricFilter, psort *selling_iface.ProductMetricSort) ([]uint64, error) {
	var err error
	var productIds []uint64
	var sortField string

	query := createReturnCancelledQuery(m.db, pfilter)

	// sorting
	switch psort.GetReturnCancelledMetricSort() {
	case product_metric.ReturnCancelledMetricSort_RETURN_CANCELLED_METRIC_SORT_LAST_CANCELLED:
		sortField = "max(its.timestamp) as sfield"
	case product_metric.ReturnCancelledMetricSort_RETURN_CANCELLED_METRIC_SORT_TOTAL_AMOUNT:
		sortField = "sum(p.piece_amount + (p.piece_count * coalesce(rc.per_piece_fee, 0))) as sfield"
	case product_metric.ReturnCancelledMetricSort_RETURN_CANCELLED_METRIC_SORT_PIECE_COUNT:
		sortField = "sum(p.piece_count) as sfield"
	case product_metric.ReturnCancelledMetricSort_RETURN_CANCELLED_METRIC_SORT_TRANSACTION_COUNT:
		sortField = "count(p.inv_transaction_id) as sfield"
	case product_metric.ReturnCancelledMetricSort_RETURN_CANCELLED_METRIC_SORT_TRANSACTION_AMOUNT:
...
func (m *returnCancelledMetric) FetchMetric(ctx context.Context, productIds []uint64, pfilter *selling_iface.ProductStatMetricFilter) (*selling_iface.ProductMetric, error) {
	var err error

	result := product_metric.ReturnCancelledMetric{
		Data: map[uint64]*product_metric.ReturnCancelledItem{},
	}

	resultList := []*product_metric.ReturnCancelledItem{}
	selects := []string{
		"p.product_id",
	...
	return &selling_iface.ProductMetric{
		Data: &selling_iface.ProductMetric_ReturnCancelledMetric{
			ReturnCancelledMetric: &result,
		},
	}, err
}
