...
type avgOrderMetric struct {
	db *gorm.DB
}

// FetchMetric implements [TeamMetricBase].
func (t *avgOrderMetric) Query(ctx context.Context, tfilter *selling_iface.TeamStatMetricFilter) (query *gorm.DB, err error) {
...
// FetchMetric implements [TeamMetricBase].
func (t *avgOrderMetric) FetchMetric(ctx context.Context, teamIds []uint64, tfilter *selling_iface.TeamStatMetricFilter) (*selling_iface.TeamMetric, error) {
	var err error
	result := team_metric.TeamAvgOrderMetric{
		Data: map[uint64]*team_metric.TeamAvgOrderItem{},
	}
	resultList := []*team_metric.TeamAvgOrderItem{}
	...
	return &selling_iface.TeamMetric{
		Data: &selling_iface.TeamMetric_TeamAvgOrderMetric{
			TeamAvgOrderMetric: &result,
		},
	}, err
}

// ProcessSort implements [TeamMetricBase].
func (t *avgOrderMetric) ProcessSort(ctx context.Context, tfilter *selling_iface.TeamStatMetricFilter, tsort *selling_iface.TeamMetricSort) ([]uint64, error) {
	var err error
	var teamIds []uint64
	var sortField string

	switch tsort.GetTeamAvgOrderMetricSort() {
	case team_metric.TeamAvgOrderMetricSort_TEAM_AVG_ORDER_METRIC_SORT_TOTAL_PER_TRANSACTION:
		sortField = "sum(o.order_mp_total)::numeric / nullif(count(o.id), 0) as sfield"
	case team_metric.TeamAvgOrderMetricSort_TEAM_AVG_ORDER_METRIC_SORT_PIECE_PER_TRANSACTION:
		sortField = "sum(coalesce(pieces.units,0))::numeric / nullif(count(o.id), 0) as sfield"
	default:
		err = errors.New("team avg order metric sort invalid sort type")
		return nil, err
	}
...
func NewAvgOrderMetric(db *gorm.DB) TeamMetricBase {
	return &avgOrderMetric{db}
}
