...
type avgOrderLostMetric struct {
	db *gorm.DB
}

// FetchMetric implements [TeamMetricBase].
func (t *avgOrderLostMetric) Query(ctx context.Context, tfilter *selling_iface.TeamStatMetricFilter) (query *gorm.DB, err error) {
...
// FetchMetric implements [TeamMetricBase].
func (t *avgOrderLostMetric) FetchMetric(ctx context.Context, teamIds []uint64, tfilter *selling_iface.TeamStatMetricFilter) (*selling_iface.TeamMetric, error) {
	var err error
	result := team_metric.TeamAvgOrderLostMetric{
		Data: map[uint64]*team_metric.TeamAvgOrderLostItem{},
	}
	resultList := []*team_metric.TeamAvgOrderLostItem{}
	...
	return &selling_iface.TeamMetric{
		Data: &selling_iface.TeamMetric_TeamAvgOrderLostMetric{
			TeamAvgOrderLostMetric: &result,
		},
	}, err
}

// ProcessSort implements [TeamMetricBase].
func (t *avgOrderLostMetric) ProcessSort(ctx context.Context, tfilter *selling_iface.TeamStatMetricFilter, tsort *selling_iface.TeamMetricSort) ([]uint64, error) {
	var err error
	var teamIds []uint64
	var sortField string

	switch tsort.GetTeamAvgOrderLostMetricSort() {
	case team_metric.TeamAvgOrderLostMetricSort_TEAM_AVG_ORDER_LOST_METRIC_SORT_TOTAL_PER_TRANSACTION:
		sortField = "sum(o.total)::numeric / nullif(count(o.id), 0) as sfield"
	case team_metric.TeamAvgOrderLostMetricSort_TEAM_AVG_ORDER_LOST_METRIC_SORT_PIECE_PER_TRANSACTION:
		sortField = "sum(coalesce(pieces.units,0))::numeric / nullif(count(o.id), 0) as sfield"
	default:
		err = errors.New("team avg order lost metric sort invalid sort type")
		return nil, err
	}
...
func NewAvgOrderLostMetric(db *gorm.DB) TeamMetricBase {
	return &avgOrderLostMetric{db}
}
