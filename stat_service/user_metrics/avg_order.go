...
type avgUserOrder struct {
	db *gorm.DB
}

// FetchMetric implements [UserMetricBase].
func (u *avgUserOrder) Query(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter) (*gorm.DB, error) {
...
// FetchMetric implements [UserMetricBase].
func (u *avgUserOrder) FetchMetric(ctx context.Context, userIds []uint64, ufilter *selling_iface.UserStatMetricFilter) (*selling_iface.UserMetric, error) {
	var err error

	result := &user_metric.UserAvgOrderMetric{
		Data: map[uint64]*user_metric.UserAvgOrderItem{},
	}
	...
	resultList := []*user_metric.UserAvgOrderItem{}
	err = query.
	...
	return &selling_iface.UserMetric{
		Data: &selling_iface.UserMetric_UserAvgOrderMetric{
			UserAvgOrderMetric: result,
		},
	}, err
}

// ProcessSort implements [UserMetricBase].
func (u *avgUserOrder) ProcessSort(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter, usort *selling_iface.UserMetricSort) ([]uint64, error) {
	var err error
	var ids []uint64
	...
	switch usort.GetUserAvgOrderMetricSort() {
	case user_metric.UserAvgOrderMetricSort_USER_AVG_ORDER_METRIC_SORT_TOTAL_PER_TRANSACTION:
		sortfield = "sum(o.order_mp_total)::numeric / nullif(count(o.id), 0) as sfield"
	case user_metric.UserAvgOrderMetricSort_USER_AVG_ORDER_METRIC_SORT_PIECE_PER_TRANSACTION:
...
func NewUserAvgOrderMetric(db *gorm.DB) UserMetricBase {
	return &avgUserOrder{db: db}
}
