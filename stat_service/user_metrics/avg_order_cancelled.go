...
type avgUserOrderCancelled struct {
	db *gorm.DB
}

// FetchMetric implements [UserMetricBase].
func (u *avgUserOrderCancelled) Query(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter) (*gorm.DB, error) {
...
// FetchMetric implements [UserMetricBase].
func (u *avgUserOrderCancelled) FetchMetric(ctx context.Context, userIds []uint64, ufilter *selling_iface.UserStatMetricFilter) (*selling_iface.UserMetric, error) {
	var err error

	result := &user_metric.UserAvgOrderCancelledMetric{
		Data: map[uint64]*user_metric.UserAvgOrderCancelledItem{},
	}
	...
	resultList := []*user_metric.UserAvgOrderCancelledItem{}
	err = query.
	...
	return &selling_iface.UserMetric{
		Data: &selling_iface.UserMetric_UserAvgOrderCancelledMetric{
			UserAvgOrderCancelledMetric: result,
		},
	}, err
}

// ProcessSort implements [UserMetricBase].
func (u *avgUserOrderCancelled) ProcessSort(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter, usort *selling_iface.UserMetricSort) ([]uint64, error) {
	var err error
	var ids []uint64
	...
	switch usort.GetUserAvgOrderCancelledMetricSort() {
	case user_metric.UserAvgOrderCancelledMetricSort_USER_AVG_ORDER_CANCELLED_METRIC_SORT_TOTAL_PER_TRANSACTION:
		sortfield = "sum(o.total)::numeric / nullif(count(o.id), 0) as sfield"
	case user_metric.UserAvgOrderCancelledMetricSort_USER_AVG_ORDER_CANCELLED_METRIC_SORT_PIECE_PER_TRANSACTION:
...
func NewUserAvgOrderCancelledMetric(db *gorm.DB) UserMetricBase {
	return &avgUserOrderCancelled{db: db}
}
