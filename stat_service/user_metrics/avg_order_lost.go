...
type avgUserOrderLost struct {
	db *gorm.DB
}

// FetchMetric implements [UserMetricBase].
func (u *avgUserOrderLost) Query(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter) (*gorm.DB, error) {
...
// FetchMetric implements [UserMetricBase].
func (u *avgUserOrderLost) FetchMetric(ctx context.Context, userIds []uint64, ufilter *selling_iface.UserStatMetricFilter) (*selling_iface.UserMetric, error) {
	var err error

	result := &user_metric.UserAvgOrderLostMetric{
		Data: map[uint64]*user_metric.UserAvgOrderLostItem{},
	}
	...
	resultList := []*user_metric.UserAvgOrderLostItem{}
	err = query.
	...
	return &selling_iface.UserMetric{
		Data: &selling_iface.UserMetric_UserAvgOrderLostMetric{
			UserAvgOrderLostMetric: result,
		},
	}, err
}

// ProcessSort implements [UserMetricBase].
func (u *avgUserOrderLost) ProcessSort(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter, usort *selling_iface.UserMetricSort) ([]uint64, error) {
	var err error
	var ids []uint64
	...
	switch usort.GetUserAvgOrderLostMetricSort() {
	case user_metric.UserAvgOrderLostMetricSort_USER_AVG_ORDER_LOST_METRIC_SORT_TOTAL_PER_TRANSACTION:
		sortfield = "sum(o.order_mp_total)::numeric / nullif(count(o.id), 0) as sfield"
	case user_metric.UserAvgOrderLostMetricSort_USER_AVG_ORDER_LOST_METRIC_SORT_PIECE_PER_TRANSACTION:
...
func NewUserAvgOrderLostMetric(db *gorm.DB) UserMetricBase {
	return &avgUserOrderLost{db: db}
}
