...
type avgUserOrderReturnCompleted struct {
	db *gorm.DB
}

// FetchMetric implements [UserMetricBase].
func (u *avgUserOrderReturnCompleted) Query(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter) (*gorm.DB, error) {
...
// FetchMetric implements [UserMetricBase].
func (u *avgUserOrderReturnCompleted) FetchMetric(ctx context.Context, userIds []uint64, ufilter *selling_iface.UserStatMetricFilter) (*selling_iface.UserMetric, error) {
	var err error

	result := &user_metric.UserAvgOrderReturnCompletedMetric{
		Data: map[uint64]*user_metric.UserAvgOrderReturnCompletedItem{},
	}
	...
	resultList := []*user_metric.UserAvgOrderReturnCompletedItem{}
	err = query.
	...
	return &selling_iface.UserMetric{
		Data: &selling_iface.UserMetric_UserAvgOrderReturnCompletedMetric{
			UserAvgOrderReturnCompletedMetric: result,
		},
	}, err
}

// ProcessSort implements [UserMetricBase].
func (u *avgUserOrderReturnCompleted) ProcessSort(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter, usort *selling_iface.UserMetricSort) ([]uint64, error) {
	var err error
	var ids []uint64
	...
	switch usort.GetUserAvgOrderReturnCompletedMetricSort() {
	case user_metric.UserAvgOrderReturnCompletedMetricSort_USER_AVG_ORDER_RETURN_COMPLETED_METRIC_SORT_TOTAL_PER_TRANSACTION:
		sortfield = "sum(o.order_mp_total)::numeric / nullif(count(o.id), 0) as sfield"
	case user_metric.UserAvgOrderReturnCompletedMetricSort_USER_AVG_ORDER_RETURN_COMPLETED_METRIC_SORT_PIECE_PER_TRANSACTION:
...
func NewUserAvgOrderReturnCompletedMetric(db *gorm.DB) UserMetricBase {
	return &avgUserOrderReturnCompleted{db: db}
}
