...
type costUserOrderCancelled struct {
	db *gorm.DB
}

// FetchMetric implements [UserMetricBase].
func (u *costUserOrderCancelled) Query(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter) (*gorm.DB, error) {
...
// FetchMetric implements [UserMetricBase].
func (u *costUserOrderCancelled) FetchMetric(ctx context.Context, userIds []uint64, ufilter *selling_iface.UserStatMetricFilter) (*selling_iface.UserMetric, error) {
	var err error

	result := &user_metric.UserCostOrderCancelledMetric{
		Data: map[uint64]*user_metric.UserCostOrderCancelledItem{},
	}
	...
	resultList := []*user_metric.UserCostOrderCancelledItem{}
	err = query.
	...
	return &selling_iface.UserMetric{
		Data: &selling_iface.UserMetric_UserCostOrderCancelledMetric{
			UserCostOrderCancelledMetric: result,
		},
	}, err
}

// ProcessSort implements [UserMetricBase].
func (u *costUserOrderCancelled) ProcessSort(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter, usort *selling_iface.UserMetricSort) ([]uint64, error) {
	var err error
	var ids []uint64
	...
	switch usort.GetUserCostOrderCancelledMetricSort() {
	case user_metric.UserCostOrderCancelledMetricSort_USER_COST_ORDER_CANCELLED_METRIC_SORT_PRODUCT_AMOUNT:
		sortfield = "sum(pieces.amount) as sfield"
	case user_metric.UserCostOrderCancelledMetricSort_USER_COST_ORDER_CANCELLED_METRIC_SORT_OWN_PRODUCT_AMOUNT:
		sortfield = "sum(pieces.own_amount) as sfield"
	case user_metric.UserCostOrderCancelledMetricSort_USER_COST_ORDER_CANCELLED_METRIC_SORT_CROSS_PRODUCT_AMOUNT:
		sortfield = "sum(pieces.cross_amount) as sfield"
	case user_metric.UserCostOrderCancelledMetricSort_USER_COST_ORDER_CANCELLED_METRIC_SORT_WAREHOUSE_AMOUNT:
		sortfield = "sum(o.warehouse_fee) as sfield"
	case user_metric.UserCostOrderCancelledMetricSort_USER_COST_ORDER_CANCELLED_METRIC_SORT_TOTAL_AMOUNT:
...
func NewUserCostOrderCancelledMetric(db *gorm.DB) UserMetricBase {
	return &costUserOrderCancelled{db: db}
}
