...
type costUserOrderOngoing struct {
	db *gorm.DB
}

// FetchMetric implements [UserMetricBase].
func (u *costUserOrderOngoing) Query(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter) (*gorm.DB, error) {
...
// FetchMetric implements [UserMetricBase].
func (u *costUserOrderOngoing) FetchMetric(ctx context.Context, userIds []uint64, ufilter *selling_iface.UserStatMetricFilter) (*selling_iface.UserMetric, error) {
	var err error

	result := &user_metric.UserCostOrderOngoingMetric{
		Data: map[uint64]*user_metric.UserCostOrderOngoingItem{},
	}
	...
	resultList := []*user_metric.UserCostOrderOngoingItem{}
	err = query.
	...
	return &selling_iface.UserMetric{
		Data: &selling_iface.UserMetric_UserCostOrderOngoingMetric{
			UserCostOrderOngoingMetric: result,
		},
	}, err
}

// ProcessSort implements [UserMetricBase].
func (u *costUserOrderOngoing) ProcessSort(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter, usort *selling_iface.UserMetricSort) ([]uint64, error) {
	var err error
	var ids []uint64
	...
	switch usort.GetUserCostOrderOngoingMetricSort() {
	case user_metric.UserCostOrderOngoingMetricSort_USER_COST_ORDER_ONGOING_METRIC_SORT_PRODUCT_AMOUNT:
		sortfield = "sum(pieces.amount) as sfield"
	case user_metric.UserCostOrderOngoingMetricSort_USER_COST_ORDER_ONGOING_METRIC_SORT_OWN_PRODUCT_AMOUNT:
		sortfield = "sum(pieces.own_amount) as sfield"
	case user_metric.UserCostOrderOngoingMetricSort_USER_COST_ORDER_ONGOING_METRIC_SORT_CROSS_PRODUCT_AMOUNT:
		sortfield = "sum(pieces.cross_amount) as sfield"
	case user_metric.UserCostOrderOngoingMetricSort_USER_COST_ORDER_ONGOING_METRIC_SORT_WAREHOUSE_AMOUNT:
		sortfield = "sum(o.warehouse_fee) as sfield"
	case user_metric.UserCostOrderOngoingMetricSort_USER_COST_ORDER_ONGOING_METRIC_SORT_TOTAL_AMOUNT:
...
func NewUserCostOrderOngoingMetric(db *gorm.DB) UserMetricBase {
	return &costUserOrderOngoing{db: db}
}
