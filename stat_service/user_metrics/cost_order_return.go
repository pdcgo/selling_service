...
type costUserOrderReturn struct {
	db *gorm.DB
}

// FetchMetric implements [UserMetricBase].
func (u *costUserOrderReturn) Query(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter) (*gorm.DB, error) {
...
// FetchMetric implements [UserMetricBase].
func (u *costUserOrderReturn) FetchMetric(ctx context.Context, userIds []uint64, ufilter *selling_iface.UserStatMetricFilter) (*selling_iface.UserMetric, error) {
	var err error

	result := &user_metric.UserCostOrderReturnMetric{
		Data: map[uint64]*user_metric.UserCostOrderReturnItem{},
	}
	...
	resultList := []*user_metric.UserCostOrderReturnItem{}
	err = query.
	...
	return &selling_iface.UserMetric{
		Data: &selling_iface.UserMetric_UserCostOrderReturnMetric{
			UserCostOrderReturnMetric: result,
		},
	}, err
}

// ProcessSort implements [UserMetricBase].
func (u *costUserOrderReturn) ProcessSort(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter, usort *selling_iface.UserMetricSort) ([]uint64, error) {
	var err error
	var ids []uint64
	...
	switch usort.GetUserCostOrderReturnMetricSort() {
	case user_metric.UserCostOrderReturnMetricSort_USER_COST_ORDER_RETURN_METRIC_SORT_PRODUCT_AMOUNT:
		sortfield = "sum(pieces.amount) as sfield"
	case user_metric.UserCostOrderReturnMetricSort_USER_COST_ORDER_RETURN_METRIC_SORT_OWN_PRODUCT_AMOUNT:
		sortfield = "sum(pieces.own_amount) as sfield"
	case user_metric.UserCostOrderReturnMetricSort_USER_COST_ORDER_RETURN_METRIC_SORT_CROSS_PRODUCT_AMOUNT:
		sortfield = "sum(pieces.cross_amount) as sfield"
	case user_metric.UserCostOrderReturnMetricSort_USER_COST_ORDER_RETURN_METRIC_SORT_WAREHOUSE_AMOUNT:
		sortfield = "sum(o.warehouse_fee) as sfield"
	case user_metric.UserCostOrderReturnMetricSort_USER_COST_ORDER_RETURN_METRIC_SORT_TOTAL_AMOUNT:
...
func NewUserCostOrderReturnMetric(db *gorm.DB) UserMetricBase {
	return &costUserOrderReturn{db: db}
}
