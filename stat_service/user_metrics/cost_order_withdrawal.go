...
type costUserOrderWithdrawal struct {
	db *gorm.DB
}

// FetchMetric implements [UserMetricBase].
func (u *costUserOrderWithdrawal) Query(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter) (*gorm.DB, error) {
...
// FetchMetric implements [UserMetricBase].
func (u *costUserOrderWithdrawal) FetchMetric(ctx context.Context, userIds []uint64, ufilter *selling_iface.UserStatMetricFilter) (*selling_iface.UserMetric, error) {
	var err error

	result := &user_metric.UserCostOrderWithdrawalMetric{
		Data: map[uint64]*user_metric.UserCostOrderWithdrawalItem{},
	}
	...
	resultList := []*user_metric.UserCostOrderWithdrawalItem{}
	err = query.
	...
	return &selling_iface.UserMetric{
		Data: &selling_iface.UserMetric_UserCostOrderWithdrawalMetric{
			UserCostOrderWithdrawalMetric: result,
		},
	}, err
}

// ProcessSort implements [UserMetricBase].
func (u *costUserOrderWithdrawal) ProcessSort(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter, usort *selling_iface.UserMetricSort) ([]uint64, error) {
	var err error
	var ids []uint64
	...
	switch usort.GetUserCostOrderWithdrawalMetricSort() {
	case user_metric.UserCostOrderWithdrawalMetricSort_USER_COST_ORDER_WITHDRAWAL_METRIC_SORT_PRODUCT_AMOUNT:
		sortfield = "sum(pieces.amount) as sfield"
	case user_metric.UserCostOrderWithdrawalMetricSort_USER_COST_ORDER_WITHDRAWAL_METRIC_SORT_OWN_PRODUCT_AMOUNT:
		sortfield = "sum(pieces.own_amount) as sfield"
	case user_metric.UserCostOrderWithdrawalMetricSort_USER_COST_ORDER_WITHDRAWAL_METRIC_SORT_CROSS_PRODUCT_AMOUNT:
		sortfield = "sum(pieces.cross_amount) as sfield"
	case user_metric.UserCostOrderWithdrawalMetricSort_USER_COST_ORDER_WITHDRAWAL_METRIC_SORT_WAREHOUSE_AMOUNT:
		sortfield = "sum(o.warehouse_fee) as sfield"
	case user_metric.UserCostOrderWithdrawalMetricSort_USER_COST_ORDER_WITHDRAWAL_METRIC_SORT_TOTAL_AMOUNT:
...
func NewUserCostOrderWithdrawalMetric(db *gorm.DB) UserMetricBase {
	return &costUserOrderWithdrawal{db: db}
}
