...
type costUserWarehouse struct {
	db *gorm.DB
}

// FetchMetric implements [UserMetricBase].
func (u *costUserWarehouse) Query(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter) (*gorm.DB, error) {
...
// FetchMetric implements [UserMetricBase].
func (u *costUserWarehouse) FetchMetric(ctx context.Context, userIds []uint64, ufilter *selling_iface.UserStatMetricFilter) (*selling_iface.UserMetric, error) {
	var err error

	result := &user_metric.UserCostWarehouseMetric{
		Data: map[uint64]*user_metric.UserCostWarehouseItem{},
	}
	...
	resultList := []*user_metric.UserCostWarehouseItem{}
	err = query.
	...
	return &selling_iface.UserMetric{
		Data: &selling_iface.UserMetric_UserCostWarehouseMetric{
			UserCostWarehouseMetric: result,
		},
	}, err
}

// ProcessSort implements [UserMetricBase].
func (u *costUserWarehouse) ProcessSort(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter, usort *selling_iface.UserMetricSort) ([]uint64, error) {
	var err error
	var ids []uint64
	...
	switch usort.GetUserCostWarehouseMetricSort() {
	case user_metric.UserCostWarehouseMetricSort_USER_COST_WAREHOUSE_METRIC_SORT_WAREHOUSE_AMOUNT:
...
func NewUserCostWarehouseMetric(db *gorm.DB) UserMetricBase {
	return &costUserWarehouse{db: db}
}
