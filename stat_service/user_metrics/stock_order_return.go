...
type stockUserOrderReturn struct {
	db *gorm.DB
}

// FetchMetric implements [UserMetricBase].
func (u *stockUserOrderReturn) Query(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter) (*gorm.DB, error) {
...
// FetchMetric implements [UserMetricBase].
func (u *stockUserOrderReturn) FetchMetric(ctx context.Context, userIds []uint64, ufilter *selling_iface.UserStatMetricFilter) (*selling_iface.UserMetric, error) {
	var err error

	result := &user_metric.UserStockOrderReturnMetric{
		Data: map[uint64]*user_metric.UserStockOrderReturnItem{},
	}
	...
	resultList := []*user_metric.UserStockOrderReturnItem{}
	err = query.
	...
	return &selling_iface.UserMetric{
		Data: &selling_iface.UserMetric_UserStockOrderReturnMetric{
			UserStockOrderReturnMetric: result,
		},
	}, err
}

// ProcessSort implements [UserMetricBase].
func (u *stockUserOrderReturn) ProcessSort(ctx context.Context, ufilter *selling_iface.UserStatMetricFilter, usort *selling_iface.UserMetricSort) ([]uint64, error) {
	var err error
	var ids []uint64
	...
	switch usort.GetUserStockOrderReturnMetricSort() {
	case user_metric.UserStockOrderReturnMetricSort_USER_STOCK_ORDER_RETURN_METRIC_SORT_PIECE_COUNT:
		sortfield = "sum(oi.count) as sfield"
	case user_metric.UserStockOrderReturnMetricSort_USER_STOCK_ORDER_RETURN_METRIC_SORT_OWN_PIECE_COUNT:
		sortfield = "sum(oi.count) filter (where oi.owned = true) as sfield"
	case user_metric.UserStockOrderReturnMetricSort_USER_STOCK_ORDER_RETURN_METRIC_SORT_CROSS_PIECE_COUNT:
		sortfield = "sum(oi.count) filter (where oi.owned = false) as sfield"
	case user_metric.UserStockOrderReturnMetricSort_USER_STOCK_ORDER_RETURN_METRIC_SORT_PIECE_AMOUNT:
		sortfield = "sum(oi.total) as sfield"
	case user_metric.UserStockOrderReturnMetricSort_USER_STOCK_ORDER_RETURN_METRIC_SORT_OWN_PIECE_AMOUNT:
		sortfield = "sum(oi.total) filter (where oi.owned = true) as sfield"
	case user_metric.UserStockOrderReturnMetricSort_USER_STOCK_ORDER_RETURN_METRIC_SORT_CROSS_PIECE_AMOUNT:
...
func NewUserStockOrderReturnMetric(db *gorm.DB) UserMetricBase {
	return &stockUserOrderReturn{db: db}
}
